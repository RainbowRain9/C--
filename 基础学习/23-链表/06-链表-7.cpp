/*
描述
把多个学生的资料（包含学号、姓名和成绩）依次存到一个链表中，每个学生的节点均放置在当前链表的合适位置，以保证链表中的每个学生以学号从小到大排列，然后遍历访问并输出所有学生的资料。
...
public:
    ...
    void addNodeByID(Student *n) // 根据 id 添加节点到链表中合适的位置
    {
        Student *p = head;
        // 从链表的头结点开始遍历
